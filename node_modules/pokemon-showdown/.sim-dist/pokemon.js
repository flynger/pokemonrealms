...
// there are very few non-malicious competitive reasons to do so
...

 {
	...
	maxMoveDisabled(baseMove) {
		...
		if (!_optionalChain([this, 'access', _4 => _4.getMoveData, 'call', _5 => _5(baseMove.id), 'optionalAccess', _6 => _6.pp])) return true;
	...
	getDynamaxRequest(skipChecks) {
		...
		if (!skipChecks) {
			...
			if (this.species.cannotDynamax || _optionalChain([this, 'access', _7 => _7.illusion, 'optionalAccess', _8 => _8.species, 'access', _9 => _9.cannotDynamax])) return;
		}
 ...
 = {
	...
	transformInto(pokemon, effect) {
		...
		if (this.battle.gen === 4) {
			...
			if (this.species.num === 493) {
				...
				const targetForme = (_optionalChain([item, 'optionalAccess', _10 => _10.onPlate]) ? 'Arceus-' + item.onPlate : 'Arceus');
	...
	disableMove(moveid, isHidden, sourceEffect) {
		...
		for (const moveSlot of this.moveSlots) {
			if (moveSlot.id === moveid && moveSlot.disabled !== true) {
				...
				moveSlot.disabledSource = (_optionalChain([sourceEffect, 'optionalAccess', _11 => _11.fullname]) || moveSlot.move);
			}
		}
	}
	...
	setStatus(
		...
		if (this.status === status.id) {
			if (_optionalChain([(sourceEffect ), 'optionalAccess', _12 => _12.status]) === this.status) {
				this.battle.add('-fail', this, this.status);
			} else if (_optionalChain([(sourceEffect ), 'optionalAccess', _13 => _13.status])) {
		...
		if (!ignoreImmunities && status.id &&
				!(_optionalChain([source, 'optionalAccess', _14 => _14.hasAbility, 'call', _15 => _15('corrosion')]) && ['tox', 'psn'].includes(status.id))) {
			// the game currently never ignores immunities
			if (!this.runStatusImmunity(status.id === 'tox' ? 'psn' : status.id)) {
				this.battle.debug('immune to status');
				if (_optionalChain([(sourceEffect ), 'optionalAccess', _16 => _16.status])) {
	...
	addVolatile(
		...
		if (!this.runStatusImmunity(status.id)) {
			...
			if (_optionalChain([(sourceEffect ), 'optionalAccess', _17 => _17.status])) {
				this.battle.add('-immune', this);
			}
			return false;
		}
